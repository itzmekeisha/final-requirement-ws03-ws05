...
$result = false;
$query = "SELECT id, name, category, quantity, uom, selling_price, description, status FROM items WHERE status = 'approved' ORDER BY id DESC";
            ...
            <tbody>
                <?php if($result && $result->num_rows > 0): ?>
                    ...
                    <?php 
                        ...
                        $clean_id = intval($row['id']);
                        $price = isset($row['selling_price']) ? floatval($row['selling_price']) : 0.00;
                    ?>
                    <tr>
                        <td style="font-weight: 500; color: #fff;"><?= htmlspecialchars($row['name'], ENT_QUOTES, 'UTF-8'); ?></td>
                        
                        <td><span style="color: #a8a29e;"><?= htmlspecialchars($row['category'], ENT_QUOTES, 'UTF-8'); ?></span></td>
                        
                        <td style="font-weight: 600; color: <?= $qty <= 5 ? '#f87171' : '#fafaf9'; ?>;">
                            <?= $qty; ?>
                            <span class="uom-badge"><?= !empty($row['uom']) ? htmlspecialchars($row['uom'], ENT_QUOTES, 'UTF-8') : 'pcs'; ?></span>
                            <?= $qty <= 5 ? ' <small style="color:#f87171; font-weight:400;">(Low)</small>' : ''; ?>
                        </td>
                        ...
                        <td class="desc-col">
                            <?= (!empty($row['description'])) ? htmlspecialchars($row['description'], ENT_QUOTES, 'UTF-8') : '<em style="color:#525252; font-style: normal;">No description</em>'; ?>
                        </td>
                        
                        <td><span class="status-badge"><?= htmlspecialchars($row['status'], ENT_QUOTES, 'UTF-8'); ?></span></td>
                        
                        <td style="text-align: center;">
                            <a href="update_item.php?id=<?= $clean_id; ?>" class="btn-edit">
